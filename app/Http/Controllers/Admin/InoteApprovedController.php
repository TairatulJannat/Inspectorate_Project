...
class InoteApprovedController extends Controller
{
    public function index()
    {
        ...
        return view('backend.inote.inote_incomming_approved.inote_approved_index', compact('inoteNew', 'inoteOnProcess', 'inoteCompleted', 'inoteDispatch'));
    }
    ...
    public function all_data(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (Auth::user()->id == 92) {
                ...
                $query = Inote::leftJoin('items', 'inotes.item_id', '=', 'items.id')
                    ->leftJoin('dte_managments', 'inotes.section_id', '=', 'dte_managments.id')
                    ->leftJoin('sections', 'inotes.section_id', '=', 'sections.id')
                    ->select('inotes.*', 'items.name as item_name',  'dte_managments.name as dte_managment_name', 'sections.name as section_name')
                ...
                ->addColumn('action', function ($data) {
                    ...
                    if ($DocumentTrack) {
                        if ($designation_id  ==  $DocumentTrack->reciever_desig_id) {
                            ...
                            <a href="' . url('admin/inote_approved/details/' . $data->id) . '" class="edit">Forward</a>
                            </div>';
                        } else {
                            $actionBtn = '<div class="btn-group" role="group">

                            <a href="' . url('admin/inote_approved/details/' . $data->id) . '" class="update">Forwarded</a>
                            </div>';
                        }
    ...
    public function details($id)
    {
        ...
        return view('backend.inote.inote_incomming_approved.inote_approved_details', compact('details', 'designations', 'document_tracks', 'desig_id', 'auth_designation_id', 'sender_designation_id',  'DocumentTrack_hidden', 'files'));
    }
    ...
    public function Tracking(Request $request)
    {
        ...
        $admin_id = Auth::user()->id;
        $section_ids = AdminSection::where('admin_id', $admin_id)->pluck('sec_id')->toArray();
        $doc_type_id = 13; //...... 13 for inote from inote table doc_serial.
